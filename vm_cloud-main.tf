...
locals {
  networks = [for k, v in var.networks : merge(v, {"mac_address" = macaddress.vm_macaddress[k].address)}]
...
resource "proxmox_virtual_environment_file" "vm_userdata" {
  ...
  source_raw {
    data      = templatefile(var.userdata_template, local.civars)
...
resource "proxmox_virtual_environment_file" "vm_netdata" {
  ...
  source_raw {
    data      = templatefile(var.netdata_template, local.civars)
