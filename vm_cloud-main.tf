...
locals {
  networks = [for k, v in var.vm_networks : merge(v, {"mac_address" = macaddress.vm_macaddress[regex("\\d",k)].address})]
...
resource "proxmox_virtual_environment_file" "vm_netdata" {
  ...
  source_raw {
    data      = templatefile(var.netdata_template, local.networks)
